...
namespace TrueSync {
     ...
     **/
    [AddComponentMenu("TrueSync/TrueSyncManager")]
    public class TrueSyncManager : MonoBehaviour {
        ...
        public static List<TSPlayerInfo> Players {
            get {
                ...
                var allPlayers = new List<TSPlayerInfo>();
                foreach (var tsp in instance.lockstep.Players.Values) {
                    if (!tsp.dropped) {
        ...
        private Dictionary<int, List<GameObject>> gameObjectsSafeMap = new Dictionary<int, List<GameObject>>();
        ...
        private TrueSyncManagedBehaviour NewManagedBehavior(ITrueSyncBehaviour trueSyncBehavior) {
            var result = new TrueSyncManagedBehaviour(trueSyncBehavior);
        ...
        private void initBehaviors() {
            ...
            foreach (var byte_player in lockstep.Players)
            {
                var playerInfo = byte_player.Value;

                var trueSyncBehaviourControllers = new List<TrueSyncManagedBehaviour>();

                foreach (var prefab in playerPrefabs)
                {
                    var prefabInst = Instantiate(prefab);
                    
                    //初始化物体Ts脚本
                    InitializeGameObject(prefabInst, prefabInst.transform.position.ToTSVector(), prefabInst.transform.rotation.ToTSQuaternion());

                    //获取物体身上所有TrueSync脚本，生成控制器，加入到trueSyncBehaviourControllers中
                    foreach (var behaviour in prefabInst.GetComponentsInChildren<TrueSyncBehaviour>())
                    {
                        behaviour.owner = playerInfo.playerInfo;
                        behaviour.localOwner = lockstep.LocalPlayer.playerInfo;
                        behaviour.numberOfPlayers = lockstep.Players.Count;

                        var tsmb = NewManagedBehavior(behaviour);
                        tsmb.owner = behaviour.owner;
                        tsmb.localOwner = behaviour.localOwner;

                        trueSyncBehaviourControllers.Add(tsmb);                        
                    }                  
                }
                behaviorsByPlayer.Add(playerInfo.ID, trueSyncBehaviourControllers);       
            }
        }

        private void initGeneralBehaviors(IEnumerable<TrueSyncManagedBehaviour> behaviours, bool realOwnerId) {
            var playersList = new List<TSPlayer>(lockstep.Players.Values);
            var itemsToRemove = new List<TrueSyncManagedBehaviour>();

            var behavioursEnum = behaviours.GetEnumerator();
            while (behavioursEnum.MoveNext()) {
                var tsmb = behavioursEnum.Current;

                if (!(tsmb.trueSyncBehavior is TrueSyncBehaviour)) {
                    continue;
                }

                var bh = (TrueSyncBehaviour)tsmb.trueSyncBehavior;
        ...
        private void CheckQueuedBehaviours() {
            if (queuedBehaviours.Count > 0) {
                ...
                for (int index = 0, length = queuedBehaviours.Count; index < length; index++) {
                    var tsmb = queuedBehaviours[index];
        ...
        public static GameObject SyncedInstantiate(GameObject prefab, TSVector position, TSQuaternion rotation) {
            if (instance != null && instance.lockstep != null) {
                var go = GameObject.Instantiate(prefab, position.ToVector(), rotation.ToQuaternion()) as GameObject;
        ...
        public static void SyncedDestroy(GameObject gameObject) {
            if (instance != null && instance.lockstep != null) {
                ...
                var tsColliders = gameObject.GetComponentsInChildren<TSCollider>();
                if (tsColliders != null) {
                    for (int index = 0, length = tsColliders.Length; index < length; index++) {
                        var tsCollider = tsColliders[index];
                        DestroyTSRigidBody(tsCollider.gameObject, tsCollider.RigidBody);
                    }
                }

                var tsColliders2D = gameObject.GetComponentsInChildren<TSCollider2D>();
                if (tsColliders2D != null) {
                    for (int index = 0, length = tsColliders2D.Length; index < length; index++) {
                        var tsCollider2D = tsColliders2D[index];
        ...
        public static void RemovePlayer(int playerId)
        {
            ...
            foreach (var tsmb in instance.behaviorsByPlayer[(byte)playerId])
            {
                tsmb.disabled = true;

                var tsColliders = ((TrueSyncBehaviour)tsmb.trueSyncBehavior).gameObject.GetComponentsInChildren<TSCollider>();
                if (tsColliders != null) {
                    for (int index2 = 0, length2 = tsColliders.Length; index2 < length2; index2++) {
                        var tsCollider = tsColliders[index2];

                        if (!tsCollider.RigidBody.TSDisabled) {
                            DestroyTSRigidBody(tsCollider.gameObject, tsCollider.RigidBody);
                        }
                    }
                }

                var tsCollider2Ds = ((TrueSyncBehaviour)tsmb.trueSyncBehavior).gameObject.GetComponentsInChildren<TSCollider2D>();
                if (tsCollider2Ds != null) {
                    for (int index2 = 0, length2 = tsCollider2Ds.Length; index2 < length2; index2++) {
                        var tsCollider2D = tsCollider2Ds[index2];
        ...
        private static void AddGameObjectOnSafeMap(GameObject go) {
            ...
            var safeMap = instance.gameObjectsSafeMap;

            var currentTick = TrueSyncManager.Ticks + 1;
        ...
        void GetLocalData(InputDataBase playerInputData) {
            ...
            if (behaviorsByPlayer.ContainsKey(playerInputData.ownerID)) {
                var managedBehavioursByPlayer = behaviorsByPlayer[playerInputData.ownerID];
                for (int index = 0, length = managedBehavioursByPlayer.Count; index < length; index++) {
                    var bh = managedBehavioursByPlayer[index];
        ...
        void OnStepUpdate(List<InputDataBase> allInputData) {
            ...
            for (int index = 0, length = generalBehaviours.Count; index < length; index++) {
                var bh = generalBehaviours[index];
        ...
        private void OnRemovedRigidBody(IBody body) {
            ...
            if (go != null) {
                var behavioursToRemove = new List<TrueSyncBehaviour>(go.GetComponentsInChildren<TrueSyncBehaviour>());
                ...
                while (behaviorsByPlayerEnum.MoveNext()) {
                    var listBh = behaviorsByPlayerEnum.Current.Value;
        ...
        private void RemoveFromTSMBList(List<TrueSyncManagedBehaviour> tsmbList, List<TrueSyncBehaviour> behaviours) {
            var toRemove = new List<TrueSyncManagedBehaviour>();
            for (int index = 0, length = tsmbList.Count; index < length; index++) {
                var tsmb = tsmbList[index];
